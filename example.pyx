...
cdef extern from "tskit.h" nogil:
    ...
    int tsk_tree_get_index(tsk_tree_t *self)
...
def iterate_trees(pyts: tskit.TreeSequence):
    ...
    while iter == 1:
        print("\ttree %d has %d roots" % (tsk_tree_get_index(&tree), tsk_tree_get_num_roots(&tree)))
        iter = tsk_tree_next(&tree)
